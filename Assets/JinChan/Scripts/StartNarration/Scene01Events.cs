...
IEnumerator EventOne()
{
    ...
    // Play faint wind sound
    if (faintWind != null)
    {
        faintWind.loop = true; // loop while typing
        faintWind.Play();
    }
    ...
    TMP_Text tmpText = textBox.GetComponent<TMP_Text>();
    yield return StartCoroutine(TypewriterEffect(tmpText, textToSpeak, 0.03f));  // typewriter effect
    ...
    IEnumerator EventThree()
    {
        ...
        TMP_Text tmpText = textBox.GetComponent<TMP_Text>();
        yield return StartCoroutine(TypewriterEffect(tmpText, textToSpeak, 0.03f, "was Liu Hai", charLiuHai, 1f));
        nextButton.SetActive(true);
        eventPos = 4;
    }
...
IEnumerator EventTen()
{
    ...
    // Optional short pause before fade
    yield return new WaitForSeconds(0.2f);

    // Activate black screen (assuming you have fadeScreenIn object)
    fadeScreenIn.SetActive(true);
    RawImage fadeImg = fadeScreenIn.GetComponent<RawImage>();
    if (fadeImg != null)
    {
        fadeImg.color = new Color(0f, 0f, 0f, 0f); // start transparent
        float fadeDuration = 2f; // duration of fade to black
        float elapsed = 0f;
